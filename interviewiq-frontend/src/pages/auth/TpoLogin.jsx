...
import { Brain, Mail, Lock, AlertCircle, Users, Briefcase, BarChart3, Settings } from 'lucide-react'
...
export default function TpoLogin() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (user.role === 'TPO') {
        ...
        setError('Unauthorized access. This portal is for Placement Officers only.')
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex bg-[#0B1120] text-white font-sans">
      ...
      <div className="hidden lg:flex flex-col justify-center px-20 w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="max-w-md">
            <h1 className="text-4xl font-bold mb-3">TPO Portal</h1>
            <p className="text-lg text-slate-400 mb-12">Track. Guide. Place.</p>

            <div className="space-y-6">
              {[
                { icon: Users, text: 'Student Tracking' },
                { icon: Briefcase, text: 'Placement Drive Management' },
                { icon: BarChart3, text: 'Analytics & Reports' },
                { icon: Settings, text: 'Cohort Management' }
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg bg-slate-800/50 border border-slate-700/50 text-blue-400 flex items-center justify-center shrink-0">
                    <item.icon size={20} />
                  </div>
      ...
      {/* Right Login Form */}
      <div className="flex-1 flex items-center justify-center p-8 bg-[#0F172A] relative">
        <div className="w-full max-w-md relative z-10">
          <div className="bg-white p-8 sm:p-10 rounded-3xl shadow-2xl">
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Login to TPO Portal</h2>
              <p className="text-slate-500 text-sm">Manage your students, track placement progress and more.</p>
            </div>
            ...
            <div className="mt-8 text-center text-sm text-slate-500">
              New here? <a href="#" className="font-semibold text-blue-600 hover:text-blue-700">Contact your college admin</a>
            </div>
          </div>
        </div>
      </div>
